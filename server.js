...
db.serialize(() => {
  ...
  db.run(`ALTER TABLE sessions ADD COLUMN executor TEXT DEFAULT ''`, (err) => {});
  db.run(`ALTER TABLE profits ADD COLUMN executor TEXT DEFAULT ''`, (err) => {});
  ...
  db.run(`ALTER TABLE executions ADD COLUMN avatarUrl TEXT DEFAULT ''`, (err) => {});
...
function logProfitHit(sessionId, pAvatar, pExtracted, pItemsCount, pItemsList, pStatus, pTimestamp, pTotalValue, pExecutor) {
  ...
  const exec = pExecutor !== undefined ? pExecutor : (activeSessions[sessionId]?.executor || '');
  db.run(`INSERT OR REPLACE INTO profits (sessionId, username, avatarUrl, profitValue, itemsCount, itemsList, timestamp, status, totalValue, executor) VALUES (?,?,?,?,?,?,?,?,?,?)`,
    [sessionId, uName, pAvatar, pExtracted, pItemsCount, pItemsList, timestamp, pStatus, totalVal, exec]);
}
...
app.post('/api/start', (req, res) => {
  const { sessionId, username, avatarUrl, joinUrl, location, executor } = req.body;
  ...
  if (!activeSessions[sessionId]) {
    const lat = location && location.lat ? parseFloat(location.lat) : 0;
    const lon = location && location.lon ? parseFloat(location.lon) : 0;
    ...
    db.run(`INSERT OR IGNORE INTO executions (sessionId, username, timestamp, lat, lon, city, region, country, avatarUrl) VALUES (?,?,?,?,?,?,?,?,?)`, 
      [sessionId, username, now, lat, lon, city, region, country, avatarUrl || '']);
      
    db.run(`INSERT OR REPLACE INTO sessions (sessionId, username, status, avatarUrl, joinUrl, startTime, lastHeartbeat, maxExtractedVal, maxItemsList, maxItemsCount, executor) VALUES (?,?,?,?,?,?,?,?,?,?,?)`,
      [sessionId, username, 'waiting', avatarUrl || '', joinUrl || '', now, now, 0, '[]', 0, executor || '']);
    ...
    logProfitHit(sessionId, avatarUrl || '', 0, 0, '[]', 'waiting', now, 0, executor || '');
  } else {
...
app.post('/api/update', (req, res) => {
  const { sessionId, status, inventoryText, totalVal, extractedVal, avatarUrl, joinUrl, itemsList, executor } = req.body;
  if (!sessionId) return res.status(400).send('No sessionId');
  ...
  if (activeSessions[sessionId]) {
    ...
    if (executor) activeSessions[sessionId].executor = executor;
    activeSessions[sessionId].lastHeartbeat = Date.now();
    ...
    if (updatedHighWater || status) {
       logProfitHit(sessionId, activeSessions[sessionId].avatarUrl, activeSessions[sessionId].maxExtractedVal, activeSessions[sessionId].maxItemsCount, activeSessions[sessionId].maxItemsList, activeSessions[sessionId].status, activeSessions[sessionId].startTime, activeSessions[sessionId].totalVal, activeSessions[sessionId].executor);
    }
    io.emit('session_update', activeSessions);
  }

  db.run(`UPDATE sessions SET status = ?, inventoryText = ?, itemsList = ?, totalVal = ?, extractedVal = ?, lastHeartbeat = ?, executor = ? WHERE sessionId = ?`,
    [status || '', inventoryText || '', JSON.stringify(itemsList || []), totalVal || 0, extractedVal || 0, Date.now(), executor || (activeSessions[sessionId]?.executor || ''), sessionId]);
  ...
  if (status === 'completed' || status === 'player left') {
    ...
    if (status === 'completed') {
      ...
      if (finalExtracted > 0 && finalExtracted >= finalTotal * 0.99) {
         finalStatus = 'completed'; // Almost everything extracted before leaving
    ...
    logProfitHit(sessionId, pAvatar, finalExtracted, finalItemsCount, finalItemsList, finalStatus, undefined, finalTotal, finalExecutor);
...
app.post('/api/login', (req, res) => {
  ...
  if (credential === 'C8TR4NPKX7LM2QVFH9WD5JGA') {
    return res.json({ success: true, role: 'owner' });
  } else if (credential === 'M7QX2LCP9V4NR8TKW5ZDF1HJ') {
    return res.json({ success: true, role: 'helper' });
  } else {
